...
######################################################################################"
    ...
    def show_classifier(self, classifier, classifier_name, hyperparameters, elapsed_time):
        try:
            ...
            self.export_button.clicked.connect(lambda: self.ouvrir_option_export(classifier))
            ...
            if self.nbr_classifier%2 == 0:
                ...
                self.export_button.setStyleSheet("border:1px solid #B6B6B6; border-radius: 4px;")
            ...
            print(f"Exception dans show_classifier: {e}")
    ...
    def export_model(self,classifier):
        try:

            if self.file_type == "pkl":
                file_path, _ = QFileDialog.getSaveFileName(self, "Save File", "", "Pickle Files (*.pkl)")
                if file_path:
                    with open(file_path, 'wb') as file: #ouverture d'un fichier binaire, ouvert en écriture ("wb" avec "b" pour binaire)
                        print(f"classifieur exporté: {classifier}")
                        pickle.dump(classifier, file)  #écriture dans le fichier

            if self.file_type == "txt":
                file_path, _ = QFileDialog.getSaveFileName(self, "Save File", "", "Text file (*.txt)")
                if file_path:
                    with open(file_path, 'a') as file:  # Ouvrez le fichier en mode 'a' pour ajouter du texte
                        print(f"classifieur exporté: {classifier}")

                        file.write("FR: Le modèle généré provient de la bibliothèque python sklearn (rendez vous sur le site de sklearn pour obtenir plus d'informations) \n"
                                   "Il contient le modèle, ainsi que les hyperparamètres que vous avec choisi. Si certains hyperparamètres ne sont pas affichés, c'est que vous avez choisi les hyperparamètres par défaut.\n"
                                   "Pour entraîner à nouveau le modèle, utilisez la méthode .fit(X,y) avec X le tableau de données et y le vecteur de labbels. Par exemple: MLPClassifier(learning_rate='invscaling', solver='sgd').fit(X,y)"
                                   "Pour prédire le label d'un jeu de donné, utilisez la méthode .predict(X), avec X votre jeu de données. Par exemple: MLPClassifier(learning_rate='invscaling', solver='sgd').predict(X) \n\n"
                                   
                                   "EN: The generated model comes from the sklearn python library (go to the sklearn website for more information)\n"
                                    "It contains the model, as well as the hyperparameters that you have chosen. If certain hyperparameters are not displayed, it is because you have chosen the default hyperparameters.\n"
                                    "To retrain the model, use the .fit(X,y) method with X the dataframe and y the associated label vector. For example: MLPClassifier(learning_rate='invscaling', solver='sgd').fit(X,y)\n"
                                    "To predict the label of a given dataset, use the .predict(X) method, with X your dataset. For example: MLPClassifier(learning_rate='invscaling', solver='sgd').predict(X) \n\n")
                        file.write(f"Model: {classifier}\n")  # Écrivez dans le fichier ouvert

        except Exception as e:
            print(f"Exception dans export_model: {e}")

    def ouvrir_option_export(self,classifier):
        try:
            self.export_window = QDialog()
            self.export_type_combo = QComboBox()
            self.export_type_combo.addItem("text file")
            self.export_type_combo.addItem("pickle file")
            self.export_type_combo.currentIndexChanged.connect(self.update_file_type)

            self.export_window_label = QLabel("Choose your export file type")

            self.export_window_ok_button = QPushButton("OK")

            self.export_window_layout = QVBoxLayout()

            self.export_window_layout.addWidget(self.export_window_label)
            self.export_window_layout.addWidget(self.export_type_combo)
            self.export_window_layout.addWidget(self.export_window_ok_button)

            self.export_window.setLayout(self.export_window_layout)

            self.export_window.setGeometry(500, 200, 300, 300)

            self.export_window_ok_button.clicked.connect(self.export_window.accept)
            self.export_window_ok_button.clicked.connect(lambda: self.export_model(classifier))
            self.export_window.exec()

        except Exception as e:
            print(f"Exception dans ouvrir_option_export :{e}")

    def update_file_type(self):
        try:
            if self.export_type_combo.currentText() == "text file":
                self.file_type = "txt"

            if self.export_type_combo.currentText() == "pickle file":
                self.file_type = "pkl"
        except Exception as e:
            print(f"exception dans update_file_type: {e}")
